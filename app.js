...
function selectHabitIcon(icon) {
    ...
    const selected = document.getElementById("selectedHabitIcon");

    if (selected) {
        selected.textContent = `Selected: ${icon}`;
    }
}
...
function createCustomHabit() {
    ...
    const nameInput =
        document.getElementById("newHabitName");

    const timeInput =
        document.getElementById("newHabitTime");

    const typeInput =
        document.getElementById("newHabitType");

    const status =
        document.getElementById("habitCreateStatus");
    ...
    if (!name) {
        ...
        showAIMessage(
            "Sir, pehle habit ka naam batayein."
        );
    ...
    if (!time) {
        ...
        showAIMessage(
            "Sir, reminder time select karein."
        );

        if (status) {
            status.textContent =
                "⚠️ Please select a reminder time.";
        }
    ...
    showAIMessage(
        `${name} is now part of your mission system.`
    );

    speak(
        `${name} has been added to your Habit Strike routine.`
    );
}
...
function loadCustomHabits() {
    ...
    const savedHabits =
        localStorage.getItem("customHabits");
    ...
    try {
        ...
        saved.forEach(function(habit) {

            // Only load real custom habits
            if (
                habit &&
                habit.custom === true
            ) {
